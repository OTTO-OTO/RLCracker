...
def load_model(data, args):
    if not args['gpu']:
        model = torch.load(args['load_file'], map_location=torch.device('cpu'), weights_only=False)
    else:
...
class ParaModel(nn.Module):
    def __init__(self, data, args, vocab, vocab_fr):
        ...
        if args.gpu_id is not None:
            self.device = torch.device(f"cuda:{args.gpu_id}" if args.gpu else "cpu")
        else:
...
class LSTM(ParaModel):
    def __init__(self, data, args, vocab, vocab_fr):
        ...
        if self.gpu:
            self.e_hidden_init = self.e_hidden_init.to(self.device)
            self.e_cell_init = self.e_cell_init.to(self.device)
