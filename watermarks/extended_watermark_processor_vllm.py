...
from watermarks.normalizers import normalization_strategy_lookup
from watermarks.alternative_prf_schemes import prf_lookup, seeding_scheme_lookup
...
class WatermarkLogitsProcessor(WatermarkBase, LogitsProcessor):
    ...
    def __call__(self, prompt_tokens_ids: List[int], past_token_ids: List[int], scores: torch.Tensor) -> torch.Tensor:
        ...
        input_ids = torch.cat([torch.LongTensor(prompt_tokens_ids), torch.LongTensor(past_token_ids)], dim=-1).unsqueeze(0).to('cuda')
        ...
        for b_idx, input_seq in enumerate(input_ids):
            if self.self_salt:
                greenlist_ids = self._score_rejection_sampling(input_seq, scores) # scores[b_idx]
            else:
        ...
        green_tokens_mask = self._calc_greenlist_mask(scores=scores.unsqueeze(0), greenlist_token_ids=list_of_greenlist_ids)
        scores = self._bias_greenlist_logits(scores=scores.unsqueeze(0), greenlist_mask=green_tokens_mask, greenlist_bias=self.delta).squeeze(0)
